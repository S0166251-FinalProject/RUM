...
class EfficientDriver(StandardDriver):
    ...
    def driveTo(self, destination, vehicle):
        ...
        route = routePlanner.determineCheapestRouteTo(destination)            
    ...
    def regulatePropulsion(self, vehicle):
        ...
        minSpeed = connection[2].getMinSpeed()*resistance
        vehicle.engineOn()       
        '''determine desired throttle and gear'''    
        desiredGear = self.determineDesiredGear(minSpeed, vehicle)
        desiredThrottle = self.determineDesiredThrottle(minSpeed, desiredGear, vehicle)
    ...
    '''CC2: 1'''     
    def determineDesiredThrottle(self, desiredSpeed, desiredGear, vehicle):
