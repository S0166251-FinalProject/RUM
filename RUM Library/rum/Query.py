...
class BasicRUMQuery(AbstractRUMQuery):
    ...
    '''returns a list of queryResults, containing the information about
    which fields are involved as operands of this operator.
    left and right might already be lists of a queryResults with field information, 
    in which case they are merged with any new fields'''
    ...
    '''checks if the current operator has boolean operands or not,
    different methods are called to determine the queryResults depending
    on the operands'''
    ...
    def  __determineNumericQueryResults__(self, guard, left, right):
        ...
        in order to get their value, the following maps need to be supplied:
        beforeVariables, afterVariables, else the variable will use the getattr function.
        ...
        if len(fieldVariables) > 0:
            ...
            component = None        
        gap = self.__determineGap__(left, right, fieldVariableValues, fieldVariableValues, component)          
    ...
    def __tryFindSolution__(self, guard, left, right, originalFieldVariableValues, adjustedFieldVariableValues, fieldVariables, gap, used, variable, step, result):
        while not self.singleResult or (len(result) == 0 and self.singleResult):
            ...
            newGap = self.__determineGap__(left, right, originalFieldVariableValues, adjustedFieldVariableValues, variable[0])
    ...
    def __determineGap__(self, left, right, oldFieldVariables, fieldVariables, component):
        if isinstance(left, AbstractQueryResult):
            left = left.getGuard()
        if isinstance(right, AbstractQueryResult):
            right = right.getGuard()
        gap = AbstractGuard.getValue(left, oldFieldVariables, fieldVariables, component, None)-AbstractGuard.getValue(right, oldFieldVariables, fieldVariables, component, None)
        return gap
    ...
    '''compares the original field variable values to the adjusted values, 
    creates QueryResults to represent the difference'''
    ...
    put them in a list of lists [component, name, step]'''
    ...
    def __processAndOrderResults__(self, results):
        ...
        if results has more than 1 entries, the root must become an OrQueryResult,
    ...
    '''adjusts the value of the given variable, adjusting it only in the supplied list,
    unless a functionvalueoperand is in play, then the variable is really modified in its component'''
    ...
    '''determines when the operator of this guard evaluates to true,
    then supplies queryResults for which this operator will return true'''
    ...
    def __determineBooleanQueryResultsHelper__(self, guard, left, right, desiredR, desiredL):
        ...
        if (isinstance(right, AbstractQueryResult)):
            ...
            if rightValue == desiredR:
                r = QueryResult(guard, [right.getComponent(), right.getName()], None, rightValue)
            else:
                r = QueryResult(guard, [right.getComponent(), right.getName()], "Set", not rightValue)               
            if rNone:
                result = r
            else:
                result.addQueryResult(r)            
